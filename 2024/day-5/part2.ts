...
function reOrder(map: Map<string, string[]>, updates: string[][]): string[][] {
  function sort(a: string, b: string): number {
    if (map.get(a)?.includes(b)) {
      return -1;
    }
    if (map.get(b)?.includes(a)) {
      return 1;
    }
    return 0;
  }
  return updates.map((up) => up.toSorted(sort));
}
...
export function part2(str: string): number {
  ...
  const fixed = reOrder(map, results);
