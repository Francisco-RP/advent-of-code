...
Deno.test.ignore("part 1 example 2", async () => {
  const testInput = `
Register A: 2024
Register B: 0
Register C: 0
...
Program: 0,3,5,4,3,0
`.trim();
  assertStrictEquals(await part1(testInput), 117440);
});

Deno.test.ignore("Part 1 still produces the accepted answer", async () => {
  const input = await Deno.readTextFile("./input.txt");
  assertStrictEquals(await part1(input), "2,7,4,7,2,1,7,5,1");
});

Deno.test.ignore("part 2 example input", async () => {
  const testInput = `
Register A: 2024
Register B: 0
Register C: 0
