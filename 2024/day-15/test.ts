...
Deno.test("part 1 example small", async () => {
  const testInput = `
########
#..O.O.#
##@.O..#
...
<^^>>>vv<v>>v<<
`.trim();
  assertStrictEquals(await part1(testInput), 2028);
});

Deno.test("part 1 example large", async () => {
  const testInput = `
##########
#..O..O.O#
#......O.#
...
`.trim();
  assertStrictEquals(await part1(testInput), 10092);
});
